...
| [`hunt.py`](hunt.py) | Kills a named NPC one kill at a time: attack, hold the safespot until it is dead, then loot once (bones first, then arrows, then value), bury, alch, repeat. Its round is a **kill**, not an attack, because a round that is one attack never knows when the fight ended and grows a cadence flag for everything after it. Measured: five moss giants in 7.5 minutes at zero food, four of five big bones buried. |
...
| [`supervisor.py`](supervisor.py) | Keeps every character **working**. `keepalive` guarantees a character stays connected; nothing guaranteed it stays busy, and those failures look identical from outside — standing still, online, healthy, achieving nothing. Reads an assignment file of character → command, checks whether that job is running, restarts it when it is not, and enforces one actor per character. |
